...
ADCON1 = 0xC0; // All channels are digital except A0 , 500 khz , right justified
}
...
void Read_Print_Control(){
  ...
  Sound_Play(2600, 100);
      ...
      myydelayms(2000);
                ...
                SpeedOfSound=(331.3+(0.606*T)); // speed of sound m/s=(331.3 + (0.606 * Tc))m/s . Tc is the temperature in Celsius .
          ...
          if(TX_Temp> 40){
                          ...
                          Speed(temp_percentage * 255);  // Maximinm pulse width (duty cycle) = 255 = maximum fan motor speed .
       ...
       // Reading Distance for both tanks using Ultrasonic module
        ...
        if(Index==2){
              ...
              if(Index==1){for(j=0;j<=50000;j++){if(PORTC&0X04){  break;}delay_us(1);}}  // If you increase the delay time more than j<=50000; that me effect on performance of   void External_Powered()
              if(Index==2){for(j=0;j<=50000;j++){if(PORTD&0X2){  break;}delay_us(1);}}

              T1CON=T1CON|0X01; // timer on

              if(check_sonic>=1 & j>=50000){k1=0; }
              if(j>=50000){check_sonic++;}
...
void main() {
     ...
     ADCON0=0B01000000; ;
     ADCON1=0B01000000;          //ALL PORTA are analog inputs
